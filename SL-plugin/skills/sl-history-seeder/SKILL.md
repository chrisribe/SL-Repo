...
Search the applicable SL index and lesson bodies plus repository instructions,
skills, and agents before capture. Mark a finding covered when equivalent
applicable guidance is already retrievable from any of those sources; cite its
owner instead of duplicating it in SL. Code comments and ordinary docs are
evidence, not automatic reasons to skip a lesson. Generated lessons are outputs,
not evidence for later findings in the same run.

## Capture

For each missing lesson:

1. Run the committed runtime's `capture` command with a searchable title, kind,
   scope/target path, and exact retrieval triggers. Use `--dry-run` first.
2. Replace the generated body only; preserve all SL-managed frontmatter. Keep the
   body concise:
   ...
   For pitfalls or mixed lessons, preserve the generated `What did NOT work`
   section and populate it. Never leave placeholder text.
...
Do not stage, commit, push, open a PR, promote, or run forgetting unless explicitly
...
remaining commit IDs and lesson paths for resumption. Historical fixes, extraction
validation, and PR approval are not verified reuse.